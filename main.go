...
import (
	"context"
	dpfm_api_caller "convert-to-dpfm-payment-requisition-mitsubishi-from-dpfm/DPFM_API_Caller"
	dpfm_api_input_reader "convert-to-dpfm-payment-requisition-mitsubishi-from-dpfm/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "convert-to-dpfm-payment-requisition-mitsubishi-from-dpfm/DPFM_API_Output_Formatter"
	"convert-to-dpfm-payment-requisition-mitsubishi-from-dpfm/config"
	"encoding/json"
	"fmt"
	"time"

	convert_complementer "convert-to-dpfm-payment-requisition-mitsubishi-from-dpfm/convert_complementer"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	errs := caller.AsyncPRMitsubishiCreates(accepter, &input, &output, l)
	if len(errs) != 0 {
